...
def all_vowel_pairs(words)
  all_vowels = "aeiou".split("")
  pairs = []

  words.each_with_index do |word, idx|
    (idx + 1...words.length).each do |compare_idx|
      combined_vowels = vowels_in_words([ words[idx], words[compare_idx] ])

      if combined_vowels.eql?(all_vowels)
        pairs << [words[idx], words[compare_idx]].join(" ") 
      end
    end
  end
...
def vowels_in_words(words_array)
  vowels = []
...
class String
    ...
    def substrings(length = nil)
      ...
      self.each_char.with_index do |char, idx|
        (idx...self.length).each do |next_idx|
          substrings << self[idx..next_idx]
        end
      end
